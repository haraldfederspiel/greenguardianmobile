...
const Home: React.FC = () => {
  ...
  const totalCarbonSaved = recentProducts.reduce(
    (total, product) => total + (product.sustainabilityScore / 100) * 2.5, 
    0
  ).toFixed(2);
  ...
  return (
    <div className="animate-fade-in">
      <header className="mb-8">
        <h1 className="text-2xl font-bold">GreenGuardian</h1>
        <p className="text-neutral-500">Your sustainable shopping assistant</p>
      </header>
      ...
      <div className="space-y-4 animate-slide-up" style={{ animationDelay: '0.1s' }}>
        {recentProducts.length > 0 ? (
          recentProducts.map(product => (
            <ProductCard key={product.id} product={product} showComparison={true} />
          ))
        ) : (
