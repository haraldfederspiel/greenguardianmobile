...
const getProductMetadata = (ingredients: string[] | undefined): { name: string, brand: string, category: string } => {
  ...
  const cosmeticIngredients = [
    'aqua', 'water', 'glycerin', 'parfum', 'fragrance', 'paraben',
    'alcohol', 'sodium', 'acid', 'extract', 'oil', 'butter', 'vitamin',
    'dimethicone', 'silica', 'titanium', 'zinc', 'oxide', 'stearate'
  ];
  
  // Food ingredients
  const foodIngredients = [
    'sugar', 'salt', 'flour', 'oil', 'water', 'milk', 'egg', 'protein',
    'vitamin', 'mineral', 'flavor', 'extract', 'powder', 'syrup', 'juice'
  ];
  
  // Check for cosmetics
  const cosmeticMatches = ingredientsList.filter(i => 
    cosmeticIngredients.some(ci => i.includes(ci))
  ).length;
  
  // Check for food
  const foodMatches = ingredientsList.filter(i => 
    foodIngredients.some(fi => i.includes(fi))
  ).length;
  
  // Determine category based on ingredient matches
  if (cosmeticMatches > foodMatches) {
    const specificNames = [
      { keywords: ['sodium laureth sulfate', 'sodium lauryl', 'cocamidopropyl'], name: 'Shampoo', brand: 'Pure Care' },
      { keywords: ['glycerin', 'paraben', 'lanolin', 'aloe'], name: 'Moisturizer', brand: 'Skin Therapy' },
      { keywords: ['titanium dioxide', 'mica', 'talc', 'silica'], name: 'Foundation', brand: 'Color Beauty' },
      { keywords: ['salicylic', 'benzoyl', 'niacinamide'], name: 'Acne Treatment', brand: 'Clear Skin' },
      { keywords: ['sodium fluoride', 'hydrated silica'], name: 'Toothpaste', brand: 'Bright Smile' }
    ];
    
    for (const type of specificNames) {
      if (ingredientsList.some(i => type.keywords.some(k => i.includes(k)))) {
        return { name: type.name, brand: type.brand, category: 'Personal Care' };
      }
    }
    
    return { name: 'Skin Care Product', brand: 'Natural Beauty', category: 'Cosmetics' };
  } else if (foodMatches > 0) {
    const specificFoods = [
      { keywords: ['corn', 'syrup', 'sugar', 'fructose'], name: 'Sweet Snack', brand: 'Tasty Treats' },
      { keywords: ['sodium', 'salt', 'monosodium'], name: 'Savory Snack', brand: 'Crunchy Bites' },
      { keywords: ['milk', 'cream', 'lactose'], name: 'Dairy Product', brand: 'Creamy Delight' },
      { keywords: ['flour', 'wheat', 'gluten'], name: 'Baked Good', brand: 'Bakery Fresh' }
    ];
    
    for (const food of specificFoods) {
      if (ingredientsList.some(i => food.keywords.some(k => i.includes(k)))) {
        return { name: food.name, brand: food.brand, category: 'Food' };
      }
    }
    
    return { name: 'Food Product', brand: 'Organic Choice', category: 'Food' };
  }
  
  // Default case
  return { name: 'Consumer Product', brand: 'Eco Choice', category: 'General' };
};
...
const generateFallbackAlternatives = (product: Product): Product[] => {
  ...
  const alternativeImages = [
    ...
    'https://images.unsplash.com/photo-1556228578-8c89e6e8ad70?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTF8fGVjbyUyMGZyaWVuZGx5fGVufDB8fDB8fHww',
    'https://images.unsplash.com/photo-1605600659873-d808a13e4d2a?w=800&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Nnx8cmVjeWNsZWQlMjBwcm9kdWN0fGVufDB8fDB8fHww'
  ];
  ...
  const getAltNamesForCategory = (category?: string) => {
    ...
    if (category.includes('food') || category.includes('snack')) {
      return [
        {name: `Organic ${category}`, brand: 'NatureFoods'},
        {name: `Sustainable ${category}`, brand: 'EcoHarvest'}
      ];
    } else if (category.includes('cosmetic') || category.includes('care') || category.includes('beauty')) {
      return [
        {name: `Natural ${category}`, brand: 'PureBeauty'},
        {name: `Eco-friendly ${category}`, brand: 'GreenGlow'}
      ];
    } else {
      return [
        {name: `Eco-friendly ${product.category || 'Product'}`, brand: 'GreenLife'},
        {name: `Recycled ${product.category || 'Product'}`, brand: 'EcoFlow'}
      ];
    }
  };
    ...
    return [
      {
        ...
        icon: Leaf,
        original: Math.min(100, Math.max(20, 100 - product.sustainabilityScore)), // Higher score = lower footprint
  ...
  if (!selectedAlternative) {
    return (
      <div className="flex flex-col items-center justify-center h-full">
        ...
        <button 
          onClick={() => navigate(-1)}
          className="bg-neutral-200 hover:bg-neutral-300 px-4 py-2 rounded-lg"
        >
          Go Back
        </button>
      </div>
